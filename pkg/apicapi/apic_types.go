...
type ApicConnection struct {
	...
	UseAPICTag        bool // use APIC tags rather than annotations
	FullSyncHook      func()
...
const aciContainersOwnerAnnotation = "owner:aci-containers-controller"
...
func (o ApicObject) SetTag(tag string, useAPICTag bool) {
	for _, body := range o {
		for j := len(body.Children) - 1; j >= 0; j-- {
			...
			for class, cbody := range body.Children[j] {
				if class == "tagAnnotation" {
					isTag = true
					if !useAPICTag && cbody.Attributes != nil {
						switch k := cbody.Attributes["key"].(type) {
						case string:
							if k == aciContainersAnnotKey {
