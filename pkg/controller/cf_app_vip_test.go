...
func TestAppVipDbOps(t *testing.T) {
...
func TestAppVipHttpHandler(t *testing.T) {
	env := testCfEnvironment(t)
	...
	var code int
	var ips []string
	code, ips = doHttpGet("1234")
	assert.Equal(t, http.StatusNotFound, code)
	assert.Nil(t, ips)

	ipdb := AppVipDb{}
	txn(env.db, func(txn *sql.Tx) {
		err := ipdb.Set(txn, "1234", "1.2.3.4", "")
		assert.Nil(t, err)
	})
	code, ips = doHttpGet("1234")
	...
	txn(env.db, func(txn *sql.Tx) {
		err := ipdb.Set(txn, "1234", "1.2.3.4", "::fe80")
		assert.Nil(t, err)
	})
	code, ips = doHttpGet("1234")
	assert.Equal(t, http.StatusOK, code)
...
func TestAppVipHttpInvalidPath(t *testing.T) {
	env := testCfEnvironment(t)
	handler := &AppVipHttpHandler{env: env}
...
func TestAppVipHttpInvalidMethod(t *testing.T) {
	env := testCfEnvironment(t)
	handler := &AppVipHttpHandler{env: env}
...
func TestAppVipHttpAuth(t *testing.T) {
	env := testCfEnvironment(t)
	cc := env.fakeCcClient()
