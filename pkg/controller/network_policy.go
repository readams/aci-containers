...
func portKey(p *v1net.NetworkPolicyPort) string {
	...
	return portProto(p.Protocol) + "-" + port
}
...
func (cont *AciController) buildServiceAugment(subj apicapi.ApicObject,
	...
	for _, prs := range portRemoteSubs {
		...
		// themselves
		for servicekey, _ := range matchedServices {
			...
			if serviceobj != nil && endpointsobj != nil {
				...
				for _, svcPort := range service.Spec.Ports {
					...
					for _, subset := range endpoints.Subsets {
						...
						}).Debug("Allowing egress for service")
					}
				}
			}
		}
	...
	for _, augment := range portAugments {
		...
		for ipstr, _ := range augment.ipMap {
...
func (cont *AciController) handleNetPolUpdate(np *v1net.NetworkPolicy) bool {
	...
	if np.Spec.PolicyTypes == nil || ptypeset[v1net.PolicyTypeEgress] {
		...
		for i, egress := range np.Spec.Egress {
			...
			// XXX revisit; creating a rule to egress to all on a
			// given port needs to enable access to any service
			// IPs/ports that have that port as their target port.
			// This will work, but it requires going through a very
			// expensive match when we could just have an index of
